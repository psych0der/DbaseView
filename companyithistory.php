...
<head>
    ...
    function deleteServices(date,ackno)
         {
            ...
            var r = confirm('Are you sure you want to delete the it history');
            if(r==true)
            {
            
             $.ajax({ 
                 url : "delete-company-ithistory.php?date="+date+"&ack="+ackno,
                async : false,
                });
            $('#'+date+ackno).remove();
            }
    ...
    $('#date').DatePicker({
      ...
      var clickEvent = function(event){
        ...
        var year = $('#year').val();
        $.get( 
             "insert_company_ithistory.php",
             { id :<?php echo $id; ?> ,date: date,ack : ack , year : year },
             function(data) {
                //alert(data);
...
<div class="add">
    ...
    <li>
         <header class="head">
			<span>
            <?php 
             echo "<a href=\"showcompany.php?id=$id\">".$company['name']." "."</a></br></br><h3><i>IT history</i></h3>";
    ...
    <?php
    $res1 = $db->select('it_history',false,'*',"`id`=$id",null);
    if($res1)
    ...
    <li id="add_button">
        <center><button class="submit bg-color-green fg-color-white"  id="create1">Add History</button>
